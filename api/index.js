...
async function initTables() {
  ...
  try {
    ...
    // Создаём таблицу channel_members
    await pool.query(`
    ...
    // Создаём таблицу channel_messages
    await pool.query(`
    ...
    // Создаём системный канал ru_news
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, token, user: { id: rows[0].id, email: rows[0].email, name: rows[0].name, nickname: rows[0].nickname } });
