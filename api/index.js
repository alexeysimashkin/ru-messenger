...
app.get('/c/:nickname', async (req, res) => {
  if (!pool) {
    return res.status(500).send('Ошибка: база не подключена');
  }
  ...
  try {
    ...
    if (!rows[0]) {
      return res.status(404).send(`
        <!DOCTYPE html>
        <html>
        <head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Канал не найден</title>
